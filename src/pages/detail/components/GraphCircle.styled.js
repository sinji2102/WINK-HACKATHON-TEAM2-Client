...
export const Circle = styled.div`
  position: absolute;
  transform: translate(calc(${({$level}) => (size[$level - 1] + stroke[$level - 1]) / 2}rem - 50%), calc(-${({$level}) => (size[$level - 1] + stroke[$level - 1]) / 2}rem - 50%));
  width: ${({$level}) => (size[$level - 1]) * 0.85}rem;
  height: ${({$level}) => (size[$level - 1]) * 0.85}rem;
  border-radius: 100%;
  background-color: ${({$color, $level, theme}) => theme.colors[`${$color}${colorLevel($level)}`]};
`;
